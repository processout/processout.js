...
(function (ProcessOut) {
    var MessageHub = (function () {
        function MessageHub(instance, onready, onnotify) {
            this.hubPath = "/messagehub.html";
            this.instance = instance;
            this.onnotify = onnotify;
            this.load(onready);
        }
        MessageHub.prototype.load = function (onready) {
            var src = this.instance.endpoint("js", this.hubPath);
            window.addEventListener("message", function (event) {
                if (!this.active)
                    return;
                var data = ProcessOut.Message.parseEvent(event);
                if (data.namespace != ProcessOut.Message.messageHubNamespace)
                    return;
        ...
        MessageHub.prototype.get = function (key) {
            if (!this.active)
                return;
            MessageHub.iframe.contentWindow.postMessage(JSON.stringify({
                "namespace": ProcessOut.Message.messageHubNamespace,
                "action": "get",
                "data": {
                    "key": key
                }
        ...
        MessageHub.prototype.set = function (key, val) {
            if (!this.active)
                return;
            MessageHub.iframe.contentWindow.postMessage(JSON.stringify({
                "namespace": ProcessOut.Message.messageHubNamespace,
    ...
    ProcessOut.MessageHub = MessageHub;
})(ProcessOut || (ProcessOut = {}));
var ProcessOut;
(function (ProcessOut) {
    var Message = (function () {
        ...
        Message.messageHubNamespace = "processout.message-hub";
        return Message;
    }());
...
(function (ProcessOut) {
    ...
    var ActionHandler = (function () {
        ...
        ActionHandler.prototype.listenEvents = function (newWindow, timer, refocus, success, error) {
            ...
            new ProcessOut.MessageHub(this.instance, null, function (k, v) {
                if (!handler)
                    return;
                var data = ProcessOut.Message.parseEvent({
                    data: v
                });
                data.namespace = k;
                handler({
