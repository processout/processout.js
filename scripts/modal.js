...
(function (ProcessOut) {
    ...
    (function (ActionFlow) {
        ...
        ActionFlow[ActionFlow["Fingerprint"] = 3] = "Fingerprint";
    ...
    var ActionHandler = (function () {
        ...
        ActionHandler.prototype.handle = function (url, success, error) {
            ...
            switch (this.options.flow) {
                case ActionFlow.NewWindow:
                    (_a = this.createNewWindowOrTab(url, true, refocus, error), topLayer = _a.topLayer, newWindow = _a.newWindow);
                    break;
                ...
                case ActionFlow.Fingerprint:
                    ...
                    break;
                default:
                    (_b = this.createNewWindowOrTab(url, false, refocus, error), topLayer = _b.topLayer, newWindow = _b.newWindow);
            }
        ...
        ActionHandler.prototype.createNewWindowOrTab = function (url, inNewWindow, refocus, error) {
            var ret = {
                topLayer: null,
                newWindow: null
            };
            if (inNewWindow) {
                ...
                ret.newWindow = window.open(url, '', "menubar=false,toolbar=false,width=" + w + ",height=" + h + ",top=" + y + ",left=" + x);
            }
            else {
    ...
    var ProcessOut = (function () {
        ...
        ProcessOut.prototype.fetchGatewayConfigurations = function (config, success, error) {
            if (!config)
                config = {};
            if (!config.invoiceID)
                throw new ProcessOut_1.Exception("processout-js.missing-invoice-id");
        ...
        ProcessOut.prototype.buildConfHookForInvoice = function (invoiceID, gatewayConf) {
