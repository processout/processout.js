...
module ProcessOut {
  ...
  export class ProcessOut {
    ...
    /**
     * Expose the NativeApm class in the instance
     * @type {NativeApm}
     */
    public nativeApm: NativeApm;
    ...
    /**
     * SetupNativeApm setups Native APM
     * @param {{ invoiceId: string; gatewayConfigurationId: string; }} options
     * @return {NativeApm}
     */
    public setupNativeApm(options: {
      invoiceId: string;
      gatewayConfigurationId: string;
    }): NativeApm {
      return new NativeApm(
        this,
        options.invoiceId,
        options.gatewayConfigurationId
      );
    }
      ...
      this.apiRequest(
        ...
        function (data: any): void {
          ...
          for (var conf of data.gateway_configurations) {
            ...
            conf.handleInvoiceAction = this.buildHandleInvoiceAction(
              config.invoiceID,
              conf
            );
    ...
    public getInvoiceActionURL(
      ...
      return this.endpoint(
        'checkout',
        `/${this.getProjectID()}/${invoiceID}/redirect/${gatewayConfID}${suffix.substring(
          0,
          suffix.length - 1
        )}`
      );
