...
module ProcessOut {
  export class Translations {
     ...
     * localeTranslationsMap. Tries the full tag first so regional variants
     * with their own table win, then the bare language, then falls back to
     * the default locale.
     */
    static resolveLocale(locale: string) {
      ...
      for (let i = 0; i < candidates.length; i++) {
        const candidate = Translations.localeAliases[candidates[i]] || candidates[i]
