...
describe("normalizePhoneValue", () => {
  ...
  it("accepts the documented `value` key in the object form", () => {
    expect(
      normalizePhoneValue({ dialing_code: "+48", value: "123123123" }),
    ).toEqual({ dialing_code: "+48", number: "123123123" })
  })
  ...
  it("returns an empty number for a non-string, non-object value", () => {
    ...
    expect(normalizePhoneValue(42)).toEqual({ dialing_code: "+44", number: "" })
  })
})
